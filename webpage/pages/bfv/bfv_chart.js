...
function printWeapons(){
    ...
    $(".variantButton").removeClass("ui-button ui-corner-all ui-widget");
...
function printWeapon(weaponStats){
    ...
    var addVariantGraphic = (weaponStats.Class == 8 || addVariantCounter != 0) ? "" : "<button class='variantButton' " + variantTooltip + ">+</button>";
...
function formatAmmoType(ammo){
    ...
    newAmmo = newAmmo.replace("75", "7.5").replace("351W", ".351 W").replace("35R", ".35 R").replace("_Fast", "").replace("_A5", "").replace("_Drilling", "").replace("_Improved", "").replace("_1897", "").replace("Incendiary", "").replace("LongRange", "").replace("_LowDrag", "").replace("Rifle", "").replace("_Aero", " ").replace("_", " ").replace("Welgun projectile", "9x19mm").replace("32ACP", ".32 ACP").replace("45cal", ".45 ACP");
    newAmmo = newAmmo.replace("_Auto", "").replace(" HighROF", "").replace("British.303", ".303 British").replace(".32 ACP Supressed", ".32 ACP").replace(".45 ACP Supressed", ".45 ACP").replace("30 _Suppressed", ".30 Carbine").replace("8x22mm", "8x22mm Nambu").replace(" 762x53mmR", "7.62×53mmR").replace("7.5x54mm", "7.5x54mm French").replace("8mmLebel", "8mm Lebel").replace("8mmRibeyrolles", "8×35mm Ribeyrolles").replace("30-06", ".30-06 Springfield").replace("7.92x33mm", "7.92x33mm Kurtz").replace("Mauser 7.92x57mm", "8mm Mauser").replace("Mauser 7x57mm", "7x57mm Mauser").replace("Mauser 65x55mm", "6.5x55mm Swedish").replace("45ACP", ".45 ACP").replace("455WebleyMkII", ".38 S&W").replace(" M3", "").replace(" MMG", "").replace(" MG", "").replace("_CMP", "").replace(" PzB39", "").replace("357Magnum", ".357 Magnum");
