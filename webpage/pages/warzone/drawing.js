function draw() {
    ...
    guyImage.onload = function () {
        ...
        watermarkImage.src = "pages/warzone/img/symWatermark.png"
    }
    guyImage.src = "pages/warzone/img/symsoldiergrey.png"
}

function drawADSBallistics(soldierImage, watermarkImage) {
    const gun = wzSelectedGuns[wzSelectedGunIndex].weaponObject;
    const { fov, adsSpread, idleSway, idleJitter, gunBob, viewBob } = gun.stats;
    const scopeMagnification = gun.stats.scopeMagnification ?? 1;
    const distanceToTarget = wzUserGameplaySettings.adsTargetDistanceMeters;
    const aimParameters = gun.aimAtTarget(distanceToTarget);
    const renderZoom = wzUserGameplaySettings.adsRenderZoom;
    const gameSettingsFoV = (fov > 30) ? fov / 80 * wzUserGameplaySettings.fov : fov;

    const finalCanvas = document.getElementById("wzADSBallisticsCanvas");
    ...
    const hFov = 4 / 3 * (wzUserGameplaySettings.useAffectedFov ? gameSettingsFoV : fov);
    ...
    function createEnvironmentalCanvas() {
        ...
        const leadDistance = aimParameters.time * wzUserGameplaySettings.targetLateralSpeed / 1000;
...
function drawHipfire(soldierImage, watermarkImage) {
    const gun = wzSelectedGuns[wzSelectedGunIndex].weaponObject;
    const canvas = document.getElementById("wzHipfireCanvas");
    const cw = canvas.width = canvas.clientWidth;
    const ch = canvas.height = canvas.clientHeight;
    const ctx = canvas.getContext("2d");
    const distance = wzUserGameplaySettings.hipTargetDistanceMeters;
    const baseFoV = wzUserGameplaySettings.fov;
